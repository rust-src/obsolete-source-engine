...
int CAI_FollowBehavior::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( failedTask == TASK_MOVE_TO_FOLLOW_POSITION || failedTask == TASK_GET_PATH_TO_FOLLOW_POSITION )
	{
		if ( m_hFollowTarget )
		{
			m_TargetMonitor.SetMark( m_hFollowTarget, m_FollowNavGoal.targetMoveTolerance * 0.5f );	
			m_FollowDelay.Start();
			NoteFailedFollow();
		}
	}
...
{
	bool bShouldUseFollowPoints = ( ShouldUseFollowPoints() && IsFollowGoalInRange( m_FollowNavGoal.followPointTolerance + 0.1f, GetGoalZRange(), GetGoalFlags() ) );
...
void CAI_FollowBehavior::OnMovementFailed()
{
	...
	if ( IsFollowGoalInRange( acceptDist * 1.5f, flZRange, GetGoalFlags() ) )
		m_bTargetUnreachable = true;
	else
		m_FollowDelay.Start();
}
...
bool CAI_FollowBehavior::FValidateHintType( CAI_Hint *pHint )
{
	...
	{
		if ( GetFollowTarget() && GetFollowTarget()->FVisible( pHint->GetAbsOrigin() + Vector( 0, 0, 0.1f ) )  )
			return true;
		else
			return false;
	}
...
constexpr inline float COMMANDER_TOLERANCE{13.0f * 1.415f};
...
bool CAI_FollowManager::RedistributeSlots( AI_FollowGroup_t *pGroup )
{
	...
	while ( ( bestSlot = FindBestSlot( pGroup ) ) != -1 && ((intp)movedFollowers.Count() < pGroup->followers.Count()) )
	{
...
{
	for ( intp i = 0; i < m_groups.Count(); i++ )
	{
...
AI_FollowGroup_t *CAI_FollowManager::FindFollowerGroup( CBaseEntity *pFollower )
{
	for ( intp i = 0; i < m_groups.Count(); i++ )
	{
		auto h = m_groups[i]->followers.Head();
