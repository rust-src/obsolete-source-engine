...
void CAI_BaseNPC::CollectShotStats( const Vector &vecShootOrigin, const Vector &vecShootDir )
{
	...
	if( ai_shot_stats.GetBool() != 0 && GetEnemy()->IsPlayer() )
	{
		...
		int iHits = 0;
		Vector initialTestDir = vecShootDir;

		CShotManipulator manipulator( initialTestDir );
	...
	if ( pOldCine && pOldCine->m_iszPlay != NULL_STRING && pOldCine->PlayedSequence() )
	{
		if ( !pOldCine->HasSpawnFlags(SF_SCRIPT_DONT_TELEPORT_AT_END) )
		{
			...
			else
			{
				...
				{
					Vector airOrigin = GetLocalOrigin();
					airOrigin.z = new_origin.z;
					SetLocalOrigin( airOrigin );
					SetGroundEntity( NULL );
				}
			}
...
void CAI_BaseNPC::ModifyOrAppendCriteria( AI_CriteriaSet& set )
{
	...
	{
		V_to_chars(buffer, gpGlobals->curtime - m_flLastSawPlayerTime);
		set.AppendCriteria( "timesinceseenplayer", buffer );
	}
	else
	{
	...
	if ( GetEnemy() )
	{
		V_to_chars(buffer, EnemyDistance(GetEnemy()));
		set.AppendCriteria( "distancetoenemy", buffer );
	}
	else
	{
