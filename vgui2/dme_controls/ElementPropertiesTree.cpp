...
void CElementTreeViewListControl::RemoveItem_R( int nItemIndex )
{
	...
	intp nCount = GetTree()->GetNumChildren( nItemIndex );
...
public:
	...
	intp OnCheckMenuItemCount() override;
...
intp CHistoryMenuButton::OnCheckMenuItemCount()
{
	Assert( m_pPropertiesTreeInternal );
	...
	BaseClass( parent, panelName ), m_pTree( tree )
{
	...
	m_pBack->SetCommand( new KeyValues( "OnNavigateBack", "item", (intp)-1 ) );
	...
	m_pFwd->SetCommand( new KeyValues( "OnNavigateForward", "item", (intp)-1 ) );
...
void CElementPropertiesTreeInternal::ApplyChanges()
{
	...
	for ( auto &w : m_AttributeWidgets )
	{
		attributewidgetfactorylist->ApplyChanges( w.m_pValueWidget, this );
	}
}
...
void CElementPropertiesTreeInternal::Refresh( RefreshType_t rebuild /* = false */, bool preservePrevSelectedItem /*= false*/ )
{
	...
	for ( auto &w : m_AttributeWidgets )
	{
		attributewidgetfactorylist->Refresh( w.m_pValueWidget, this );
	}
}
...
void CElementPropertiesTreeInternal::GetPathToItem( CUtlVector< TreeItem_t > &path, intp itemIndex )
{
	for ( intp idx = itemIndex; idx != m_pTree->GetTree()->GetRootItemIndex(); idx = m_pTree->GetTree()->GetItemParent( idx ) )
	{
...
int CElementPropertiesTreeInternal::OpenPath( const CUtlVector< TreeItem_t > &path )
{
	...
	for ( const TreeItem_t &childTreeItem : path )
	{
		...
		{
			intp nChildIndex = m_pTree->GetTree()->GetChild( itemIndex, i );
...
void CElementPropertiesTreeInternal::OnPaste_( bool reference )
{
	...
	if ( isArrayElement || attType == AT_ELEMENT_ARRAY )
	{
		...
		intp nArrayIndex = isArrayElement ? itemData->GetInt( "arrayIndex" ) + 1 : array.Count();
...
void CElementPropertiesTreeInternal::OnDeleteSelected()
{
	...
	intp nSelected = selection.Count();
	if ( !nSelected )
		return;
...
bool CElementPropertiesTreeInternal::OnRemoveFromData( CUtlVector< KeyValues * >& list )
{
	...
	for ( auto i = sorted.m_Sorted.FirstInorder(); i != sorted.m_Sorted.InvalidIndex(); i = sorted.m_Sorted.NextInorder( i ) )
	{
		OwnerAttribute_t& entry = sorted.m_Sorted[ i ];

		// Walk it backward by array index...
		for ( auto j = entry.sortedData.LastInorder(); j != entry.sortedData.InvalidIndex(); j = entry.sortedData.PrevInorder( j ) )
		{
...
void CElementPropertiesTreeInternal::OnSortByName()
{
	...
	for ( intp i = 0 ; i < c; ++i )
	{
		...
		intp nCount = elementArray.Count();
		if ( nCount == 0 )
			continue;
...
#else
void CElementPropertiesTreeInternal::OnNavBack( uint64 item )
#endif
{
	intp c = m_hHistory.Count();
	if ( c <= 1 )
		return;
...
#else
void CElementPropertiesTreeInternal::OnNavForward( uint64 item )
#endif
{
	intp c = m_hHistory.Count();
	if ( c <= 0 )
	...
	)
{
	...
	intp nAttributes = element->AttributeCount();
...
// 	SetObject( m_SearchResultsRoot.Get() );
	...
	intp nIndex = m_pTree->GetTree()->GetRootItemIndex();
	intp c = expandIndices.Count();
	for ( intp i = c - 2; i >= 0 ; --i )
	{
		intp idx = expandIndices[ i ];

		// Expand the item
		m_pTree->ExpandItem( nIndex, true );

#ifdef _DEBUG
		intp children = m_pTree->GetTree()->GetNumChildren( nIndex );
		if ( idx >= children ) 
		{
			Assert( 0 );
			break;
...
#endif
		intp childIndex = m_pTree->GetTree()->GetChild( nIndex, idx );
		nIndex = childIndex;
	}
...
void CElementPropertiesTreeInternal::OnNavSearch( const char *text )
{
	...
	if ( changed )
	{
		...
		if ( m_SearchResultsRoot.Get() )
		{
			...
			intp c = m_SearchResults.Count();

			char sz[ 512 ];
			Q_snprintf( sz, sizeof( sz ), "Search Results [%zd] for '%s'", c, m_szSearchStr );
	...
	m_nCurrentSearchResult = clamp( m_nCurrentSearchResult, (intp)0, m_SearchResults.Count() - 1 );
...
intp CElementPropertiesTreeInternal::GetHistoryMenuItemCount( int whichMenu )
{
	intp c = m_hHistory.Count();
	if ( !c )
		return 0;
...
void CElementPropertiesTreeInternal::PopulateHistoryMenu( int whichMenu, Menu *menu )
{
	...
	case DME_PROPERTIESTREE_MENU_BACKWARD:
		{
			for ( intp i = m_nCurrentHistoryPosition + 1; i < c; ++i )
			{
	...
	case DME_PROPERTIESTREE_MENU_FORWARD:
		{
			for ( intp i = 0 ; i < m_nCurrentHistoryPosition; ++i )
			{
...
void CElementPropertiesTreeInternal::GenerateContextMenu( int itemIndex, [[maybe_unused]] int x, [[maybe_unused]] int y )
{
	...
	int id = m_hContextMenu->AddMenuItem( "#DmeElementPropertiesPaste", new KeyValues( "OnPaste" ), this );
...
void CElementPropertiesTreeInternal::DropItemsIntoArray( CDmrElementArray<> &array, CUtlVector< KeyValues* > &msglist, CUtlVector< CDmElement* > &list, intp nArrayIndex, DropOperation_t op )
{
	intp nElements = list.Count();
	if ( op == DO_COPY )
	{
	...
	else if ( op == DO_MOVE )
	{
		...
		intp nMsgs = msglist.Count();
		for ( intp i = 0; i < nMsgs; ++i )
		{
		...
		for ( auto i = arrayItemSorter.LastInorder(); i != arrayItemSorter.InvalidIndex(); i = arrayItemSorter.PrevInorder( i ) )
		{
	...
	intp base = array.InsertMultipleBefore( nArrayIndex, nElements );
	...
	if ( op == DO_MOVE )
	{
		if ( isArrayElement || attType == AT_ELEMENT_ARRAY )
		{
			...
			for ( intp i = 0; i < nElements; ++i )
			{
				...
				{
					intp nChildItem = m_pTree->GetTree()->GetChild( itemIndex, ci );
...
int CElementPropertiesTreeInternal::FindTreeItem( int nParentIndex, const TreeItem_t &info )
{
	...
	{
		intp nChildIndex = m_pTree->GetTree()->GetChild( nParentIndex, i );
...
void CElementPropertiesTreeInternal::BuildOpenItemList( OpenItemTree_t &tree, int nParent, int nItemIndex, bool preservePrevSelectedItem )
{
	...
	if ( selected )
	{
		...
		intp preserve = preservePrevSelectedItem 
	...
	// Deal with children
	intp nCount = m_pTree->GetTree()->GetNumChildren( nItemIndex );
	for ( intp i = 0; i < nCount; ++i )
	{
		intp nChildIndex = m_pTree->GetTree()->GetChild( nItemIndex, i );
...
void CElementPropertiesTreeInternal::RefreshTreeItemState( int nItemID )
{
	...
	intp nChildCount = m_pTree->GetTree()->GetNumChildren( nItemID );
	for ( intp i = 0; i < nChildCount; ++i )
	{
		intp nChildID = m_pTree->GetTree()->GetChild( nItemID, i );
...
void CElementPropertiesTreeInternal::InsertAttributes( int parentNodeIndex, CDmElement *obj )
{
	...
	AttributeInfo_t *pInfo = stackallocT( AttributeInfo_t, obj->AttributeCount() );
	intp nCount = 0;
